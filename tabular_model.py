...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        conv1 = self.conv1(x)       # (16,32,32,32)
        conv2 = self.conv2(conv1)   # (32,16,16,16)
        conv3 = self.conv3(conv2)   # (64,8,8,8)
        conv4 = self.conv4(conv3)   # (16,4,4,4)
        return conv4
...
class LSP(nn.Module):
    def __init__(self, num_neighbours, dims, agg_method, gpu,
                  activation="leakyrelu", dropout=0.0, slope=0.2, batch_norm=True):
        super(LSP, self).__init__()

        self.encoder = self.build_layers(dims[:dims.index(min(dims)) + 1], activation, dropout, slope, batch_norm)
        self.decoder = self.build_layers(dims[dims.index(min(dims))::-1], activation, dropout, slope, batch_norm, reverse=True)

        self.num_neighbours = num_neighbours
        self.agg_method = agg_method
        self.gpu = gpu

    def build_layers(self, layer_dims, activation, dropout, slope, batch_norm, reverse=False):
        layers = []
    ...
    def forward(self, tab1, tab2):
        bs = tab1.shape[0]
        tab1 = tab1.view(tab1.shape[0], -1)
        tab2 = tab2.view(tab2.shape[0], -1)

        zs = self.encoder(torch.cat([tab1, tab2], 0))
        recons = self.decoder(zs)
        z1, z2 = zs[:bs], zs[bs:]
        recon1, recon2 = recons[:bs], recons[bs:]
    ...
    def build_graph_batch(self, zs):
        ...
        if self.num_neighbours < bs:
            ...
            for i in range(bs):
                ks = torch.argsort(dis_mx[i], descending=False)[:self.num_neighbours+1] # get the index of the nearest neighbours
    ...
    def build_graph_dataset(self, zs_all, zs):
        ...
        if self.num_neighbours < bs:
            print("num_neighbours < bs")
            adj_mx_filter = torch.zeros(bs, ds).to(self.gpu)
            for i in range(bs):
                ks = torch.argsort(dis_mx[i], descending=False)[:self.num_neighbours+1]     # Take the five nearest neighbors, including the sample itself.
                adj_mx_filter[i, ks] = adj_mx[i, ks]
            return adj_mx_filter
        else:
            print("num_neighbours >= bs")
            return adj_mx * (1. - torch.eye(bs, ds).to(self.gpu))
...
class LSPEncoderWrapper(nn.Module):
    def __init__(self, lsp_model: nn.Module, method, device='cuda'):
        super().__init__()
        self.lsp_model = lsp_model.eval()
        self.device = device
        self.method = method
        self.pca = None

    def forward(self, tab1_input: torch.Tensor, latent_index: int):
        tab2_input = torch.zeros_like(tab1_input).to(self.device)
        interval = None  # or dummy tensor if required by your model

        # Remove torch.no_grad() so gradients can flow!
        [z1, _], _ = self.lsp_model(tab1_input, tab2_input, interval)

        if self.method == 'latent_features':
            return z1[:, latent_index]
        elif self.method == 'Euclidean':
            return torch.norm(z1[:], dim=1)
        elif self.method == 'PCA':
            pc_vector = torch.tensor(self.pca.components_[latent_index], dtype=torch.float32).to(self.device)
            return torch.matmul(z1, pc_vector)
